...
def typed_submission_correction(tools: list[Any], response: ModelResponse[Any], error: str) -> str:
    ...
    schema = next(
        (
            ...
            for tool in tools
            if getattr(tool, "name", None) == "submit_answer"
        ),
        None,
    )
    ...
    instructions = (
        ...
        if "answer" in fields
        else "For guide answers, title is a short label, summary contains the explanation, "
        "and suggested_questions is an array. Do not put the whole response in title."
    )
    return (
        "Your attempted submit_answer arguments failed validation: "
        + error
        + ". Call submit_answer exactly once using its JSON schema. "
        + instructions
...
def valid_skill_call(messages: list[Any], skill: str) -> bool:
    ...
    skill_batches = [calls for calls in batches if any(call.get("name") == "read_file" for call in calls)]
...
class EvidenceSubmissionRepair:
    ...
    def _remaining(self, request: ModelRequest[Any]) -> tuple[frozenset[tuple[str, str]], bool]:
        ...
        completed = frozenset(
            ...
            if (
                getattr(record, "identity", None) is not None
    ...
    async def correct(
        ...
        tools = [item for item in request.tools if getattr(item, "name", None) == next_name]
        ...
        for attempt in range(2):
            ...
            chosen = [
                call for message in repaired.result for call in (getattr(message, "tool_calls", None) or ())
            ]
            if (
                len(chosen) == 1
                and chosen[0].get("name") in allowed
                and (
                    not strict
                    or (
                        chosen[0].get("name"),
