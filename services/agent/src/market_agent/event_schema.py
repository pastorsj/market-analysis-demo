...
class MarketRequirement(ExactModel):
    ...
    def bounded(self):
        _need(1 <= len(self.required_fields) <= 2 and tuple(sorted(set(self.required_fields))) == self.required_fields, "event_market_requirement")
        return self
...
class DocumentRequirement(ExactModel):
    ...
    @model_validator(mode="after")
    def bounded(self):
        _need(1 <= len(self.source_kinds) <= 3 and tuple(sorted(set(self.source_kinds))) == self.source_kinds, "event_document_requirement")
        return self
...
class DerivedRequirement(ExactModel):
    required_for_ready: Literal[True]
    features: tuple[Literal[
        "event-returns", "relative-returns", "volume-context", "historical-analogues",
    ], ...]

    @model_validator(mode="after")
    def bounded(self):
        order = ("event-returns", "relative-returns", "volume-context", "historical-analogues")
        _need(1 <= len(self.features) <= 4 and tuple(item for item in order if item in self.features) == self.features, "event_derived_requirement")
        return self
...
class Qualification(ExactModel):
    ...
    def consistent(self):
        layers = (
            ("market", self.market), ("documents", self.documents),
            ("licensed_news", self.licensed_news), ("derived_features", self.derived_features),
        )
        _need(self.market.status == "ready", "event_market_not_ready")
        for name, layer in layers:
            allowed = {"ready", "blocked"} if name == "market" else {"ready", "partial"}
            _need(layer.status in allowed and (layer.status == "ready") == (not layer.gaps), "event_layer_status")
...
class Event(ExactModel):
    ...
    event_session: date; source_dates: tuple[date, ...]
    ...
    @model_validator(mode="after")
    def bounded(self):
        _need(1 <= len(self.analysis_tickers) <= 5 and len(set(self.analysis_tickers)) == len(self.analysis_tickers), "event_analysis_scope")
        _need(self.analysis_tickers[0] == self.primary_ticker, "event_primary_scope")
        _need(1 <= len(self.context_instruments) <= 4 and len(set(self.context_instruments)) == len(self.context_instruments), "event_context_scope")
        _need(not set(self.analysis_tickers) & set(self.context_instruments), "event_scope_overlap")
        _need(all(_TICKER.fullmatch(item) for item in (*self.analysis_tickers, *self.context_instruments)), "event_ticker")
        _need(self.start_session <= self.event_session <= self.end_session and self.event_session <= self.default_cutoff.date() <= self.end_session, "event_chronology")
        _need(1 <= len(self.source_dates) <= 8 and len(set(self.source_dates)) == len(self.source_dates) and all(self.start_session <= item <= self.default_cutoff.date() for item in self.source_dates), "event_source_dates")
        _need(3 <= len(self.questions) <= 6 and len({item.question_id for item in self.questions}) == len(self.questions), "event_questions")
        _need(1 <= len(self.limitations) <= 20 and len({item.limitation_id for item in self.limitations}) == len(self.limitations), "event_limitations")
        return self
...
class PreparedCatalog(ExactModel):
    ...
    def consistent(self):
        ...
        _need(tuple(item.sort_order for item in self.categories) == tuple(range(1, len(self.categories) + 1)), "event_category_order")
        ...
        _need(tuple(item.sort_order for item in self.events) == tuple(sorted(item.sort_order for item in self.events)), "event_order")
        ...
        _need(self.summary.published_events == len(self.events) and self.summary.ready_events == ready and self.summary.partial_events == len(self.events) - ready, "event_summary")
        _need(self.summary.excluded_events == len(self.excluded_events) and self.summary.declared_events == len(self.events) + len(self.excluded_events), "event_summary")
        return self
