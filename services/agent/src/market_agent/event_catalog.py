...
_UNSAFE = re.compile(r"(?:\bllama(?:\b|[-_/])|(?:https?|grpc)://|\bbearer\s+|\b(?:api[_ -]?key|authorization|password)\b|\b(?:secret|token)\s*=)", re.I)
...
_SOURCE_DATE_QUESTION = re.compile(r"\b(?:evidence|filing|news|release|report(?:ed)?|results|source|statement)\b", re.I)
...
_NON_TICKER_REFERENCES = frozenset({
    "BROADER", "FINANCIAL", "HISTORY", "ITS", "MARKET", "RECENT", "SECTOR",
    "THE", "THEIR", "VOLATILITY", "VOLUME",
})
_SCOPE_REFERENCES = frozenset({"EVENT", "MOVE", "PEER", "PEERS", "RESULTS", "SHOCK"})
_COMPANY_ALIASES = {
    "ADVANCED": "AMD", "AMD": "AMD", "BROADCOM": "AVGO", "CHIPMAKER": "AVGO",
    "CHARLES": "SCHW", "GOLDMAN": "GS", "JP": "JPM", "JPMORGAN": "JPM",
    "NVIDIA": "NVDA", "SCHWAB": "SCHW",
...
class ShockEventCatalog:
    ...
    def load(cls, root: Path, coverage: CoverageCatalog) -> "ShockEventCatalog":
        ...
        catalog_value, catalog_body = _read(resolved / "catalog.json", maximum=8 * 1024 * 1024, code="event_catalog")
        try:
        ...
        _need(record.sha256 == prepared_sha and record.bytes == len(catalog_body) and record.records == len(document.events), "event_artifact_digest")
        _need(manifest.binding == document.binding and manifest.summary == document.summary, "event_artifact_projection")
        expected_id = "shock-events-" + _digest(_canonical({
        ...
        _need(document.binding.scenario_manifest_sha256 == coverage.scenario_manifest_sha256, "event_scenario_binding")
        _need(document.binding.market_snapshot_id == coverage.market_snapshot_id, "event_scenario_binding")
        _need(document.binding.market_manifest_sha256 == coverage.market_manifest_sha256, "event_scenario_binding")
        _need(document.binding.document_snapshot_id == coverage.document_snapshot_id, "event_scenario_binding")
        _need(document.binding.document_manifest_sha256 == coverage.document_manifest_sha256, "event_scenario_binding")
...
def bind_event_request(request: InvestigationRequest, events: ShockEventCatalog | None) -> tuple[InvestigationRequest, Event]:
    ...
    if event is None:
        raise EventRequestError("That known shock event is unavailable. Choose a listed event or use custom scope.")
    if request.ticker is not None and request.ticker != event.primary_ticker:
        raise EventRequestError(f"This event uses {event.primary_ticker} as its primary ticker. Clear the conflicting ticker or use custom scope.")
    if request.as_of is not None and not _same_cutoff(request.as_of, event.default_cutoff):
        raise EventRequestError(f"This event uses the evidence cutoff {event.default_cutoff.isoformat().replace('+00:00', 'Z')}. Clear the conflicting cutoff or use custom scope.")
...
def resolve_bound_policy(
    ...
    *,
    ...
    clauses = re.split(r"\b(?:and|while)\b|[,;?]", canonical.question, flags=re.I); market_dates = {day for clause in clauses if _MARKET_DATE_QUESTION.search(clause) or re.search(r"\bcompare\b", clause, re.I) and not _SOURCE_DATE_QUESTION.search(clause) for day in parse_question(clause, coverage).dates}
    allowed_dates = {event.event_session, event.default_cutoff.date(), *event.source_dates}; parsed_dates_compatible = not parsed.dates or len(parsed.dates) <= 2 and set(parsed.dates) <= allowed_dates and market_dates <= {event.event_session}
    foreign_ticker_mentions = tuple(
        ticker for ticker in parsed.unknown_tickers
        if ticker not in _NON_TICKER_REFERENCES and re.search(
    ...
    foreign_focus_mentions = tuple(
        ...
        if token not in _SCOPE_REFERENCES
        and _COMPANY_ALIASES.get(token, token) not in named_allowed
    )
    ...
    event_resolved_reasons = {
        PolicyReason.SCOPE_CONFLICT, PolicyReason.MISSING_DATE,
    ...
    bounded_event_scope = (
        ...
        and (
            authored_question
            or bool(_GENERIC_EVENT_SCOPE.search(canonical.question))
        )
    )
    if parsed.dates and not parsed_dates_compatible and decision.kind not in {
        PolicyKind.REFUSAL, PolicyKind.PRODUCT_HELP, PolicyKind.CONVERSATION,
    }:
    ...
    if (
        ...
        and (
            ...
            and (
                decision.reason in event_resolved_reasons
                or bounded_event_scope
            )
    ...
    label = (
        ...
        f"{event.default_cutoff.astimezone(timezone.utc).isoformat().replace('+00:00', 'Z')}."
        f"{qualification}"
    )
    ...
    return replace(decision, request=canonical, scope=scope, requested_day=event.event_session, effective_session_day=event.event_session)


__all__ = [
    "EventCatalogError", "EventRequestError", "ShockEventCatalog",
    "bind_event_request", "resolve_bound_policy",
]
