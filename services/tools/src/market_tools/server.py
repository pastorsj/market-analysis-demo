...
def _receipt(result: object, family: str, bundle: ArtifactBundle) -> None:
    receipt = _attr(result, "receipt"); store = bundle.store
    if store is None:
        raise StartupProofError("scenario_store_missing")
    expected = {
        "engine": family, "gpu_executed": True, "fallback_used": False,
    ...
    if receipt is None or not _attr(receipt, "device") or any(_attr(receipt, key) != value for key, value in expected.items()):
...
def prove_runtime(bundle: ArtifactBundle, candidate: ToolEngine) -> dict[str, str]:
    ...
    store = bundle.store
    if store is None or store.manifest.get("schema_version") != 2 or store.manifest.get("recipe_version") != 3:
        raise StartupProofError("scenario_store_missing")
    ...
    for symbol in ("NVDA", "JPM"):
        result = candidate.get_price_context(symbol, cutoff); _receipt(result, "cudf", bundle)
        if not _attr(result, "evidence"):
            raise StartupProofError("cudf_gpu_probe_failed")
    semantic = candidate.search_news("NVDA", cutoff, "runtime readiness", top_k=2); _receipt(semantic, "cuvs", bundle)
    graph = candidate.trace_shock_propagation("NVDA", cutoff, max_depth=1); _receipt(graph, "cugraph", bundle)
    risk = candidate.predict_volatility_risk("NVDA", cutoff); _receipt(risk, "xgboost-gpu", bundle)
    topics = candidate.project_news_topics("NVDA", cutoff, dimensions=2, max_documents=4); _receipt(topics, "cuml", bundle)
    if not _attr(semantic, "evidence") or not _attr(graph, "data").get("paths") or not _attr(topics, "artifacts"):
...
def _gaps(values: object) -> list[dict[str, object]]:
    ...
    for value in values if isinstance(values, list) else []:
        ...
        for key in ("instrument_id", "issuer_id", "source_kind", "event_id", "required"):
            ...
            if isinstance(candidate, (bool, int)) or isinstance(candidate, str) and re.fullmatch(r"[A-Za-z0-9_.:-]{1,80}", candidate):
                item[key] = candidate
        safe.append(item)
    return safe
...
def safe_health(bundle: ArtifactBundle, probes: dict[str, str]) -> dict[str, object]:
    value = bundle.health()
    keys = (
        "scenario_id", "scenario_manifest_sha256", "market_snapshot_id", "market_manifest_sha256",
        "document_snapshot_id", "document_manifest_sha256", "market_readiness_sha256",
        "document_readiness_sha256", "data_tier", "vintage_status", "targets",
        "required_benchmarks", "optional_instruments", "date_coverage", "field_coverage",
        "document_coverage", "semantic", "risk_model",
    )
...
def _publish(**values: object) -> None:
    state.clear(); state.update(values)
...
async def lifespan(_server: MCPServer):
    ...
    try:
        ...
        candidate = ToolEngine(bundle, strict_gpu=settings.strict_gpu, embed_path=settings.embed_path if settings.strict_gpu else None)
...
@mcp.tool(description="Detect abnormal return, volume and volatility for one stock/date.", annotations=read_only, structured_output=True)
def detect_market_shock(ticker: str, as_of: datetime) -> ToolResult:
    return _engine().detect_market_shock(ticker, as_of)


@mcp.tool(description="Compare a company with semiconductor and broad-market benchmarks.", annotations=read_only, structured_output=True)
def get_price_context(ticker: str, as_of: datetime) -> ToolResult:
    return _engine().get_price_context(ticker, as_of)


@mcp.tool(description="Retrieve cutoff-qualified company reporting and primary sources.", annotations=read_only, structured_output=True)
...
@mcp.tool(description="Find quantitative historical patterns without claiming causal identity.", annotations=read_only, structured_output=True)
...
@mcp.tool(description="Trace bounded, cited company and market relationship paths.", annotations=read_only, structured_output=True)
...
@mcp.tool(description="Estimate five-session annualized realized volatility.", annotations=read_only, structured_output=True)
def predict_volatility_risk(ticker: str, as_of: datetime) -> ToolResult:
    return _engine().predict_volatility_risk(ticker, as_of)


@mcp.tool(description="Create a bounded two- or three-dimensional evidence topic map.", annotations=read_only, structured_output=True)
def project_news_topics(ticker: str, as_of: datetime, dimensions: int = 2, max_documents: int = 99) -> ToolResult:
...
async def health(_request: Request) -> JSONResponse:
    return JSONResponse({"service": "tools", "live": True, **state}, status_code=200 if state["ready"] else 503)
