...
def _receipt(result: dict[str, Any], operation: str, ticker: str) -> dict[str, object]:
    ...
    if operation not in OPERATIONS or not isinstance(value.get("engine"), str) or not isinstance(value.get("device"), str):
        raise DashboardProjectionError("invalid_receipt")
    duration = _number(value.get("duration_ms"))
    if (duration is None or duration < 0 or type(value.get("gpu_executed")) is not bool
            or value.get("fallback_used") is not False
            or (value["engine"] == "deterministic") == value["gpu_executed"]):
        raise DashboardProjectionError("invalid_receipt")
    return {"operation": operation, "ticker": ticker, "engine": value["engine"],
            "device": value["device"], "gpu_executed": value["gpu_executed"],
            "fallback_used": False, "duration_ms": duration}
...
def _limitations(result: dict[str, Any]) -> list[dict[str, object]]:
    ...
    for row in result.get("limitations", []):
        ...
        if (isinstance(item.get("code"), str) and isinstance(item.get("message"), str)
                and isinstance(affected, list) and all(isinstance(value, str) for value in affected)):
...
def _merge_limitations(values: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for row in values:
        current = merged.setdefault(str(row["code"]), {"code": row["code"], "message": row["message"], "affected": []})
...
def _series(engine: object, ticker: str, benchmark: str, cutoff: datetime, resolved: str) -> tuple[dict, list[dict]]:
    ...
    for symbol, fields in ((ticker, ("adjusted_close", "volume")), (benchmark, ("adjusted_close",))):
        ...
        if window.status != "ready" or not window.rows or not window.gpu_executed or window.resolved_session != resolved:
            raise DashboardProjectionError("series_gpu_unavailable")
        if any(str(row.get("session_date", "")) > resolved or _utc(row.get("bar_end"), "invalid_series_time") > cutoff for row in window.rows):
            raise DashboardProjectionError("future_series_value")
        windows[symbol] = {row["session_date"]: row for row in window.rows}
        receipts.append({"operation": "market_series", "ticker": symbol, "engine": "cudf",
                         "device": str(engine.device), "gpu_executed": True,
                         "fallback_used": False, "duration_ms": duration})
    ...
    for session in sessions:
        ...
        close, benchmark_close = _number(target.get("adjusted_close")), _number(reference.get("adjusted_close"))
        if close is None or benchmark_close is None or close <= 0 or benchmark_close <= 0:
            raise DashboardProjectionError("invalid_series_value")
        volume = _number(target.get("volume"))
        points.append({"session_date": session, "ticker_close": close, "benchmark_close": benchmark_close,
...
def _evidence(result: dict[str, Any], cutoff: datetime) -> list[dict[str, object]]:
    ...
    for item in result.get("citations", [])[:4]:
        row = _wire(item); available = _utc(row.get("available_at"), "invalid_evidence_time")
        if available > cutoff:
        ...
        if not all(isinstance(row.get(key), str) and row[key] for key in ("citation_id", "evidence_id", "title", "excerpt")):
            raise DashboardProjectionError("invalid_evidence")
        values.append({"citation_id": row["citation_id"], "title": row["title"],
...
def build_dashboard(engine: object, ticker: str, as_of: str) -> dict[str, object]:
    ...
    store = engine.bundle.store; sessions = store.sessions
    ...
    receipts, limits, watchlist = [], [
        {"code": "historical_reconstruction_not_historical_vintage", "message": "Market prices were reconstructed later and are not an archived-at-cutoff feed.", "affected": ["market_data"]},
        {"code": "company_evidence_not_licensed_news", "message": "Company evidence excludes licensed news in this reconstruction.", "affected": ["company_evidence"]},
    ], []
    for symbol in TARGETS:
        price = _result(engine, "get_price_context", symbol, cutoff)
        shock = _result(engine, "detect_market_shock", symbol, cutoff)
        receipts.extend((_receipt(price, "get_price_context", symbol), _receipt(shock, "detect_market_shock", symbol)))
        limits.extend((*_limitations(price), *_limitations(shock)))
        performance = next((row for row in price.get("data", {}).get("performance", []) if row.get("ticker") == symbol), None)
        price_session = (performance or {}).get("resolved_session") or price.get("data", {}).get("resolved_session")
        ...
        outcome = "no_data" if price_outcome == "no_data" else "partial" if price_outcome != "ok" or shock_outcome != "ok" else "ok"
        values = performance or {}; shock_values = {} if outcome == "no_data" else shock.get("data", {})
        watchlist.append({"ticker": symbol, "outcome": outcome, "resolved_session": price_session,
                          "close": _number(values.get("close")), "return_1_session_pct": _number(values.get("return_1_session_pct")),
                          "return_5_sessions_pct": _number(values.get("return_5_sessions_pct")), "opening_gap_pct": _number(values.get("opening_gap_pct")),
                          "volume_ratio": _number(values.get("volume_ratio")), "benchmark": benchmark,
                          ...
                          "is_shock": shock_values.get("is_shock") if type(shock_values.get("is_shock")) is bool else None})
    selected = next(row for row in watchlist if row["ticker"] == ticker)
    series, series_receipts = _series(engine, ticker, str(selected["benchmark"]), cutoff, resolved); receipts.extend(series_receipts)
    evidence_result = _result(engine, "search_news", ticker, cutoff)
    receipts.append(_receipt(evidence_result, "search_news", ticker)); limits.extend(_limitations(evidence_result))
    return {"schema_version": "market-dashboard-v1", "mode": "historical_reconstruction",
            "requested_as_of": requested.isoformat(), "resolved_session": resolved,
            "cutoff_at": cutoff.isoformat().replace("+00:00", "Z"), "selected_ticker": ticker,
            "coverage": {"first_session": first, "last_session": last, "session_count": len(sessions),
                         "scenario_id": store.manifest["scenario_id"], "vintage_status": store.manifest["vintage_status"]},
            "watchlist": watchlist, "series": series, "evidence": _evidence(evidence_result, cutoff),
            "receipts": receipts, "limitations": _merge_limitations(limits)}
