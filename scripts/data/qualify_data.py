...
CRITICAL_READINESS = {
    "market": {
        "advanced-001": "ready",
        "advanced-020": "needs_scope_resolution",
        "shock-005": "ready",
        "multi-007": "ready",
        "guardrail-009": "ready",
    },
...
def _stable_regular_bytes(
    ...
    *,
    ...
    try:
        ...
        parent_identities.append((current, tuple(getattr(root_metadata, field) for field in identity_fields)))
        for part in relative.parts[:-1]:
            current /= part
            metadata = os.lstat(current)
            if not stat.S_ISDIR(metadata.st_mode) or stat.S_ISLNK(metadata.st_mode):
                raise RuntimeError(code)
            parent_identities.append((current, tuple(getattr(metadata, field) for field in identity_fields)))
    ...
    try:
        for parent, expected in parent_identities:
            ...
            if not stat.S_ISDIR(metadata.st_mode) or stat.S_ISLNK(metadata.st_mode) or observed != expected:
...
def _runtime_images_receipt(runtime_root: Path) -> tuple[dict[str, Any], bytes]:
    ...
    for service, expected_name in RUNTIME_IMAGE_NAMES.items():
        ...
        expected_fields = {"name", "id"} | ({"build_input_sha256"} if service != "model" else set())
        if (
...
class MCPCaller:
    def __init__(self, url: str):
        ...
        response = self.client.post(
            ...
            json={
                ...
                "params": {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "phase09-qualifier", "version": "1"},
                },
            },
        )
        ...
        self.client.post(
            url, headers=self.headers, json={"jsonrpc": "2.0", "method": "notifications/initialized"}
        )
        self.identifier = 1
...
def _document_readiness_expectations(rows: list[dict[str, Any]]) -> tuple[dict[str, int], bool]:
    ...
    for row in rows:
        ...
        if not required:
            ...
            derived = status
            valid = valid and all(
                code.startswith("unsupported_") or code == "event_requirement_unavailable" for code in codes
            )
        else:
            valid = False
            continue
...
def _limitations_valid(value: Any) -> bool:
    ...
    for item in value:
        ...
        if (
            ...
            or not 1 <= len(item["message"].strip()) <= 500
...
def _outcome_valid(
    outcome: Any, coverage: list[dict[str, Any]], limitations: list[dict[str, Any]], artifacts: list[Any]
) -> bool:
    ...
    if outcome == "no_data":
        return bool(
            limitations
            and not artifacts
            and any(item["required"] and item["status"] == "missing" for item in coverage)
        )
    return False
...
def _tool_result_valid(value: Any, tool: str) -> bool:
    ...
    if (
        ...
        or not isinstance(value["data"].get("summary"), str)
        or not value["data"]["summary"].strip()
        ...
        or not _outcome_valid(
            value.get("outcome"), value["coverage"], value["limitations"], value["artifacts"]
        )
    ):
        return False
...
def _tool_observation(
    ...
    return {
        ...
        "as_of": value["as_of"],
        "request": _plain(arguments),
        "coverage": coverage,
        "coverage_sha256": _digest(coverage),
        "limitations": limitations,
        "limitations_sha256": _digest(limitations),
        "data": data,
...
def _recorded_tool_valid(value: Any, tool: str, inputs: dict[str, Any]) -> bool:
    ...
    try:
        ...
        return False
    return bool(
        _qualification_outcome_valid(value)
        and value.get("tool") == tool
        and value.get("schema_version") == "2.0"
        and timestamp.tzinfo is not None
        and isinstance(value.get("request"), dict)
        and value["request"].get("as_of") == value.get("as_of")
        and _coverage_valid(value.get("coverage"))
        and _limitations_valid(value.get("limitations"))
        ...
        and isinstance(value["data"].get("summary"), str)
        and bool(value["data"]["summary"].strip())
...
def _publication_receipt(
    ...
    if candidate_mode:
        ...
        supplied = _absolute(
            publication_receipt if publication_receipt.is_absolute() else runtime_root / publication_receipt
        )
        if supplied != expected:
...
def _expected_inputs(
    ...
    market = json.loads(
        (scenario_root / manifest["market"]["path"] / "manifest.json").read_text(encoding="utf-8")
    )
    documents = json.loads(
        (scenario_root / manifest["documents"]["path"] / "manifest.json").read_text(encoding="utf-8")
    )
...
def _query_dicts(
    connection: duckdb.DuckDBPyConnection, query: str, values: list[Any]
) -> list[dict[str, Any]]:
...
def _market_capture_controls(
    runtime_root: Path, market_root: Path, manifest: dict[str, Any]
) -> list[dict[str, Any]]:
    ...
    for source in manifest["sources"]:
        ...
        if (
            root.name != capture_id
            or not raw_manifest.is_file()
            or raw_manifest.read_bytes() != lineage.read_bytes()
        ...
        if capture.get("adapter") == "kaggle_cc0":
            ...
            if (
                ...
                not in {
                    "version_pinned_api",
                    "operator_supplied_pinned_archive",
                    "verified_legacy_cache",
                }
                or metadata_mode
                not in {
                    "current_dataset_api",
                    "operator_supplied_current_dataset_metadata",
                    "verified_legacy_cache",
                }
                or (
                    "verified_legacy_cache" in {request_mode, metadata_mode} and request_mode != metadata_mode
                )
                or capture.get("sanitized_urls")
                != {
                    "metadata": profile["metadata_url"],
                    "download": profile["download_url"],
            ...
            if (
                ...
                != capture.get("metadata_stable_sha256")
            ):
...
def _market_observation(
    ...
    session_values: list[dict[str, str]],
    runtime_root: Path,
) -> dict[str, Any]:
    symbols = (
        market_manifest["universe"]["targets"]
        + market_manifest["universe"]["peers"]
    ...
    actions = _query_dicts(
        connection,
        "SELECT * FROM read_parquet(?) ORDER BY instrument_id,effective_at",
    ...
    return {
        ...
        "quality": market_manifest["quality"],
        "universe": market_manifest["universe"],
        "benchmark_policy": market_manifest["benchmark_policy"],
        "sources": market_manifest["sources"],
        "capture_lineage": sorted(lineage, key=lambda row: row["source_id"]),
...
def _document_observation(
    ...
    coverage_statuses = _query_dicts(
        connection,
        """
    ...
    for source in document_manifest["sources"]:
        ...
        descriptors += [
            capture[key] for key in ("file", "records_artifact", "gaps_artifact") if capture.get(key)
        ]
        observed = []
    ...
    return {
        "direct_rows": connection.execute(
            "SELECT count(*) FROM read_parquet(?)", [documents_path]
        ).fetchone()[0],
        "groups": groups,
        "coverage_rows": connection.execute(
            "SELECT count(*) FROM read_parquet(?)", [coverage_path]
        ).fetchone()[0],
        "coverage_statuses": coverage_statuses,
        "coverage": document_manifest["observed_coverage"],
        "gaps": document_manifest["gaps"],
        "requirements": document_manifest["requirements"],
        "sources": document_manifest["sources"],
    ...
    for name in ("market", "documents"):
        ...
        result[name] = {
            ...
            "unsupported": [
                ...
                for row in rows
                if row["status"] in {"expected_missing", "needs_scope_resolution", "blocked"}
            ],
        }
    return result
...
def _control(
    connection: duckdb.DuckDBPyConnection, pattern: str, ticker: str, benchmark: str, session: str
) -> dict[str, Any]:
    ...
    return {
        "close": float(target[-1][1]),
        "volume": int(target[-1][2]),
        "return_pct": target_return,
        "benchmark_return_pct": benchmark_return,
        "market_adjusted_return_pct": target_return - benchmark_return,
...
def _parity_checks(values: dict[str, Any], tools: list[dict[str, Any]], tolerance: float) -> dict[str, bool]:
    ...
    return {
        ...
        "benchmark_return": _close(
            actual["benchmark_return_pct"], expected["benchmark_return_pct"], tolerance
        ),
        "relative_return": _close(
            actual["market_adjusted_return_pct"], expected["market_adjusted_return_pct"], tolerance
        ),
...
def _environment_observation(tools_url: str) -> dict[str, Any]:
    ...
    try:
        ...
        default_route = None
    return {
        "observation_source": "container_runtime",
        "containerized": Path("/.dockerenv").exists(),
        "tools_scheme": parsed.scheme,
        "tools_host": parsed.hostname,
        "tools_address": address,
...
def _isolated_environment(value: Any) -> bool:
    try:
        ...
        return False
    return bool(
        value.get("containerized") is True
        and value.get("tools_scheme") == "http"
        and value.get("tools_host") == "tools"
        and internal
...
def _seeded_requests(
    ...
    generator = random.Random(20250913)
    requests = []
    for index in range(samples):
        ...
        requests.append(
            {
                "ticker": ticker,
                "benchmark": benchmark_policy[sector]["required"][0],
...
def qualify(
    ...
    *,
    ...
    input_binding = _expected_inputs(
        ...
        scenario_root,
        manifest,
        _tools_health_identity(tools_url),
        publication_receipt,
    )
    ...
    report = {
        "schema_version": 1,
        "phase": "09-data-recovery",
        "status": "pass" if not failures else "fail",
        "run_id": hashlib.sha256(
            f"{manifest['scenario_id']}:{started.isoformat()}:{samples}".encode()
        ).hexdigest()[:20],
        ...
        "vintage_status": manifest["vintage_status"],
        "inputs": input_binding,
        "market": market_observation,
        "documents": document_observation,
        "readiness": readiness,
        "semantic": manifest["semantic"],
        "parity": {
            "seed": 20250913,
            "requested": samples,
            "completed": len(parity),
            "tolerance": 1e-6,
            "samples": parity,
        },
        "latency_ms": {
            "samples": latencies,
            "p50": _quantile(latencies, 0.5) if latencies else None,
        ...
        "release_eligibility": (
            ...
            if gate == "release"
                ...
                "status": "not_eligible",
                "reasons": [
                    "current-capture reconstruction",
                    "no entitled archived-at-cutoff market input supplied",
                ],
            }
        ),
    }
    ...
    try:
        return {
            "market": _market_observation(
                connection,
                str(market_root / "bars/interval=1d/year=*/part-*.parquet"),
                market_root,
                market_manifest,
                sessions,
                runtime_root,
...
def _expected_parity_controls(
    ...
    try:
        return {
            int(item["sample"]): _control(
                connection, pattern, item["ticker"], item["benchmark"], item["session"]
            )
            for item in samples
        }
    finally:
...
def _validate_report_value(
    ...
    *,
    ...
    if (
        ...
        or report["gate"]
        != ("reconstruction" if manifest["data_tier"] == "cc0_reconstruction" else "release")
    ):
    ...
    if (
        ...
        or parity.get("seed") != 20250913
        or parity.get("requested", 0) < 100
        or parity.get("completed") != len(samples)
        or parity.get("tolerance") != 1e-6
    ):
    ...
    if (
        ...
        or len(parity_ordinals) != len(samples)
        or len(failure_ordinals) != len(sampled_failures)
        or any(
            item.get("code") not in {"readiness_contract", "parity_mismatch", "control_or_tool_error"}
    ...
    for item in samples:
        ...
        if (
            ...
            or not all(
                _recorded_tool_valid(value, tool, report["inputs"])
                for value, tool in zip(
                    observations,
                    ("get_price_context", "detect_market_shock"),
                    strict=True,
                )
            )
        ):
        ...
        if observed_request != expected_request or any(
            value.get("request") != expected_args for value in observations
        ):
    ...
    expected = {
        "p50": _quantile(latency_samples, 0.5) if latency_samples else None,
        "p95": _quantile(latency_samples, 0.95) if latency_samples else None,
    ...
    if report["gate"] == "reconstruction":
        ...
        if (
            ...
            or any(
                report["market"].get("symbols", {}).get(symbol, {}).get(field) != 0
                for symbol in targets
                for field in raw_fields
            )
            or "KRE" not in optional
            or not any(
                item.get("code") == "optional_instrument_unavailable" and item.get("instrument_id") == "KRE"
    ...
    if (
        ...
        or environment.get("tools_scheme") != "http"
        or not environment.get("tools_host")
        or not environment.get("tools_address")
        or not isinstance(environment.get("containerized"), bool)
...
def main() -> int:
    ...
    if args.output is None:
        args.output = (
            ...
            else args.root / "reports/data/phase-09-qualification.json"
        )
    ...
    print(
        json.dumps(
            {"status": report["status"], "run_id": report["run_id"], "failures": len(report["failures"])}
        )
    )
