...
def semantic_index_valid(semantic: Any, index: Any, identifiers: list[str], *, risk_sha256: str) -> bool:
    ...
    expected = {
        ...
        "index_path": "indexes/cuvs-brute-force.bin",
        "index_sha256": semantic.get("index_sha256"),
        "gpu_executed": True,
        "scenario_input_sha256": semantic.get("scenario_input_sha256"),
        "market_manifest_sha256": semantic.get("market_manifest_sha256"),
        "document_manifest_sha256": semantic.get("document_manifest_sha256"),
        "risk_model_metadata_sha256": risk_sha256,
    ...
    exact_types = (
        type(receipt.get("gpu_executed")) is bool
        and type(receipt.get("fallback_used")) is bool
        and type(receipt.get("dimension")) is int
        and type(index.get("dimension")) is int
        and type(index.get("rows")) is int
        and all(type(index.get(key)) is bool for key in ("normalized", "fixture_only", "gpu_executed"))
    )
    return (
        exact_types
        and receipt == receipt_expected
        and receipt["device"] == "NVIDIA GB10"
        and index.get("device") == receipt["device"]
        and index.get("attention") in {"flash_attention_2", "sdpa"}
...
def _npy_sections(value: bytes) -> list[tuple[str, tuple[int, ...], bytes]] | None:
    ...
    try:
        while offset < len(value):
            ...
            data_end = header_end + count * sizes[dtype]
            if data_end > len(value):
                return None
            sections.append((dtype, shape, value[header_end:data_end]))
            offset = data_end
...
def _semantic_bytes_valid(vectors: bytes, binary: bytes, rows: int) -> bool:
    ...
    if sys.byteorder != "little":
        values.byteswap()
    if len(values) != rows * 2048 or any(not math.isfinite(value) for value in values):
        return False
    if any(
        abs(math.sqrt(math.fsum(value * value for value in values[start : start + 2048])) - 1.0) > 2e-3
    ...
    if (
        binary[:4] != b"<f4\0"
        or sections is None
        or [(kind, shape) for kind, shape, _ in sections] != expected
    ):
        return False
...
def semantic_artifacts_valid(artifacts: Any, eligible_count: Any, *, root: Path) -> bool:
    ...
    if not isinstance(artifacts, list) or type(eligible_count) is not int or eligible_count <= 0:
        return False
    selected = {
        path: [row for row in artifacts if isinstance(row, dict) and row.get("path") == path]
        for path in (
    ...
    metadata_valid = (
        type(ids.get("records")) is int
        and ids["records"] == eligible_count
        and type(vectors.get("records")) is int
        and vectors["records"] == eligible_count
        and type(vectors.get("bytes")) is int
        and vectors["bytes"] == eligible_count * 2048 * 4
        and type(binary.get("records")) is int
        and binary["records"] == 1
        and type(binary.get("bytes")) is int
        and binary["bytes"] > 0
        and type(metadata.get("records")) is int
        and metadata["records"] == 1
    )
    if not metadata_valid:
        return False
    try:
        resolved = root.resolve()
        vector_path = (resolved / "indexes/embeddings.f32").resolve()
        binary_path = (resolved / "indexes/cuvs-brute-force.bin").resolve()
        if resolved not in vector_path.parents or resolved not in binary_path.parents:
            return False
        vector_bytes, binary_bytes = vector_path.read_bytes(), binary_path.read_bytes()
    except OSError:
        return False
    return (
        len(vector_bytes) == vectors["bytes"]
        and len(binary_bytes) == binary["bytes"]
...
def _utc(value: Any, field: str, issues: list[ArtifactIssue]) -> datetime | None:
    if not isinstance(value, str) or not value.endswith("Z"):
        issues.append(ArtifactIssue("invalid_utc_time", field))
        return None
    try:
...
def validate_source_manifest(payload: Any) -> None:
    if not isinstance(payload, dict) or set(payload) != {"version", "sources"} or payload.get("version") != 1:
        raise ArtifactValidationError(
            [ArtifactIssue("invalid_source_manifest", "expected version 1 and sources only")]
        )
    ...
    for index, source in enumerate(sources):
        if not isinstance(source, dict) or not required.issubset(source) or not set(source).issubset(allowed):
            issues.append(ArtifactIssue("invalid_source", str(index)))
            continue
        source_id, digest = source["id"], source["content_sha256"]
        if source_id in seen:
            issues.append(ArtifactIssue("duplicate_source", source_id))
        seen.add(source_id)
        if not str(source["canonical_url"]).startswith("https://"):
            issues.append(ArtifactIssue("unsafe_source_url", source_id))
        if source["redistribution"] not in {"allowed", "metadata_only", "local_only", "blocked"}:
            issues.append(ArtifactIssue("invalid_redistribution", source_id))
        if (
            not isinstance(digest, str)
            or len(digest) != 64
            or any(c not in "0123456789abcdef" for c in digest)
        ):
            issues.append(ArtifactIssue("invalid_source_digest", source_id))
        retrieved = _utc(source["retrieved_at"], f"{source_id}.retrieved_at", issues)
        published = (
            _utc(source.get("published_at"), f"{source_id}.published_at", issues)
            if source.get("published_at")
            else None
        )
        if retrieved and published and retrieved < published:
            issues.append(ArtifactIssue("time_inversion", source_id))
    if issues:
...
def load_manifest(
    ...
    *,
    ...
    try:
        ...
        raise ArtifactValidationError([ArtifactIssue("manifest_unreadable", str(exc))]) from exc
    if (
        ...
        return _load_v2(
            ...
            bundle_root,
            payload,
            allow_test_embeddings=allow_test_embeddings,
    ...
    if not isinstance(payload, dict) or set(payload) != MANIFEST_KEYS:
        issues.append(ArtifactIssue("manifest_shape", "unknown or missing fields"))
    if (
        payload.get("schema_version") != 1
        or payload.get("scenario_id") != "deepseek-nvda-v1"
        or payload.get("snapshot_id") != "market-shock-corpus-v1"
    ):
        issues.append(ArtifactIssue("manifest_identity", "unexpected identity"))
    _utc(payload.get("created_at"), "created_at", issues)
    coverage = payload.get("date_coverage", {})
    if (
        not isinstance(coverage, dict)
        or set(coverage) != {"start", "end", "timezone", "calendar"}
        or coverage.get("timezone") != "America/New_York"
        or coverage.get("calendar") != "XNYS"
    ):
        issues.append(ArtifactIssue("coverage_contract", "calendar mismatch"))
    embedding = payload.get("embedding", {})
    if not isinstance(embedding, dict) or set(embedding) != EMBED_KEYS:
        issues.append(ArtifactIssue("embedding_shape", "unknown or missing fields"))
    else:
        expected = {
            "model_id": EMBED_MODEL,
            "revision": EMBED_REVISION,
        ...
        if embedding.get("implementation") != "pinned_model" and not (
            allow_test_embeddings
            and embedding.get("implementation") == "deterministic_fake_test_only"
            and payload.get("fixture_tier") == "ci"
        ):
            issues.append(ArtifactIssue("test_embedding_forbidden", str(embedding.get("implementation"))))
    artifacts = payload.get("artifacts")
    if not isinstance(artifacts, list) or len(artifacts) < 6:
        issues.append(ArtifactIssue("artifact_count", "at least six required"))
        artifacts = []
    seen: set[str] = set()
    for item in artifacts:
        if not isinstance(item, dict) or set(item) != ARTIFACT_KEYS:
            issues.append(ArtifactIssue("artifact_shape", repr(item)))
            continue
        rel = Path(item["path"])
        if rel.is_absolute() or ".." in rel.parts or not rel.parts or rel.parts[0] not in ALLOWED_ROOTS:
            issues.append(ArtifactIssue("artifact_path", item["path"]))
            continue
        if item["path"] in seen:
            issues.append(ArtifactIssue("duplicate_artifact", item["path"]))
        seen.add(item["path"])
        candidate = (bundle_root / rel).resolve()
        if bundle_root not in candidate.parents:
            issues.append(ArtifactIssue("artifact_escape", item["path"]))
        elif not candidate.is_file():
            issues.append(ArtifactIssue("artifact_missing", item["path"]))
        else:
            if candidate.stat().st_size != item["bytes"]:
                issues.append(ArtifactIssue("artifact_size", item["path"]))
            if sha256_file(candidate) != item["sha256"]:
                issues.append(ArtifactIssue("artifact_digest", item["path"]))
    if issues:
        raise ArtifactValidationError(issues)
    return payload
...
def _safe_child(root: Path, relative: str, issues: list[ArtifactIssue]) -> Path | None:
    ...
    if item.is_absolute() or not item.parts or ".." in item.parts:
        issues.append(ArtifactIssue("artifact_path", relative))
        return None
    candidate = (root / item).resolve()
    if root not in candidate.parents:
        issues.append(ArtifactIssue("artifact_escape", relative))
        return None
    return candidate
...
def _readiness_rows(
    ...
    for row in rows:
        ...
        seen.add(case_id)
        if row.get(snapshot_key) != binding.get("snapshot_id") or row.get(manifest_key) != nested_binding.get(
            "manifest_sha256"
        ):
        ...
        if (
            ...
            and any(
                (
                    row.get("tickers"),
                    row.get("required_instruments"),
                    row.get("optional_instruments"),
                    row.get("required_fields"),
                    row.get("required_interval") is not None,
        ...
        if name == "market":
            blocking = [
                ...
                if item.get("code")
                not in {
                    "optional_instrument_unavailable",
                    "subdaily_chronology_unavailable",
                }
            ]
    ...
    if len(seen) != 250 or len(bank_digests) != 1 or len(contract_digests) != 250:
        issues.append(
            ArtifactIssue(
                "readiness_set_integrity",
                f"{name}:cases={len(seen)}:banks={len(bank_digests)}:contracts={len(contract_digests)}",
            )
        )
    return rows
...
def _validate_derived(
    ...
    *,
    ...
    try:
        ...
        issues.append(ArtifactIssue("derived_artifact_unreadable", type(exc).__name__))
        return
    for name, item in (
        ("graph", graph),
        ("analogue", analogue),
    ...
    if (
        graph.get("cutoff_policy") != "valid_from_lte_query_cutoff"
        or not isinstance(graph_sources, list)
        or any(
            not row.get("source_id")
            or not row.get("source_row_id")
            or row.get("input_source_ids") != graph_sources
    ...
    if (
        analogue.get("cutoff_policy") != "feature_at_lt_query_cutoff"
        or not isinstance(analogue_rows, list)
        or any(
            not row.get("source_id")
            or not row.get("source_row_id")
            or not row.get("input_source_row_ids")
            or row.get("future_outcome_excluded") is not True
            or any(
                not math.isfinite(float(row[key]))
                for key in ("return_1d", "absolute_return_pct")
                if row.get(key) is not None
            )
            or row.get("volume_ratio") is not None
            and not math.isfinite(float(row["volume_ratio"]))
    ...
    if (
        ...
        or training.get("label_contract") != label_contract
        or not isinstance(training_rows, list)
        or any(
            row.get("feature_at", "") >= row.get("label_available_at", "")
            or row.get("label_available_at", "") > str(cutoff)
            or len(row.get("label_source_row_ids", [])) != 5
            or not row.get("label_source_ids")
    ...
    normalized = [
        json.loads(line) for line in (root / "processed/documents.jsonl").read_text().splitlines() if line
    ]
    expected_projection = [
        {
            "chunk_id": row["chunk_id"],
            "issuer_ids": row["issuer_ids"],
    ...
    if (
        projection.get("cutoff_policy") != "published_at_lte_query_cutoff"
        or projection.get("documents") != expected_projection
    ):
    ...
    if (
        risk.get("schema_version") != 2
        or risk.get("input_binding") != binding
        ...
        or risk.get("training_cutoff") != cutoff
        or risk.get("training_rows") != len(training_rows)
        or risk.get("output_contract") != expected_output
    ):
        issues.append(ArtifactIssue("risk_model_binding", "metadata"))
        return
    ...
    if risk.get("model_path") != "risk-model.ubj" or risk.get("receipt_path") != "risk-model.receipt.json":
        issues.append(ArtifactIssue("risk_model_path", "model/receipt"))
        return
    ...
    try:
        ...
        receipt = {}
    if (
        risk.get("status") != "ready"
        or not model_path.is_file()
        or not receipt_path.is_file()
        or risk.get("model_sha256") != sha256_file(model_path)
        or risk.get("model_bytes") != model_path.stat().st_size
        or risk.get("receipt_sha256") != sha256_file(receipt_path)
        or receipt.get("input_binding") != binding
        or receipt.get("training_input_sha256") != risk.get("training_input_sha256")
        or receipt.get("training_rows") != len(training_rows)
        or receipt.get("training_cutoff") != cutoff
        or receipt.get("model_sha256") != risk.get("model_sha256")
        or receipt.get("model_bytes") != risk.get("model_bytes")
        or receipt.get("algorithm") != "xgboost.XGBRegressor"
        or not receipt.get("algorithm_version")
...
def _load_v2(
    ...
    *,
    ...
    if (
        payload.get("data_tier") == "cc0_reconstruction"
        and payload.get("vintage_status") != "reconstructed_later"
    ):
    ...
    if not isinstance(embedding, dict):
        ...
        if (
            type(embedding.get("dimension")) is not int
            or type(embedding.get("max_tokens")) is not int
            or type(embedding.get("normalized")) is not bool
        ):
        ...
        allowed_implementation = (
            ...
            or (allow_pending_semantic and implementation == "pending_gpu")
            or (
                allow_test_embeddings
                and implementation == "deterministic_fake_test_only"
    ...
    for name, binding in (("market", market), ("documents", documents)):
        if not isinstance(binding, dict) or set(binding) != {"path", "snapshot_id", "manifest_sha256"}:
            issues.append(ArtifactIssue("nested_binding_shape", name))
            continue
        ...
        if not manifest_path.is_file():
            issues.append(ArtifactIssue("nested_manifest_missing", name))
            continue
        if sha256_file(manifest_path) != binding["manifest_sha256"]:
            issues.append(ArtifactIssue("nested_manifest_digest", name))
            continue
        try:
            if name == "market":
                from scripts.data.market_contract import validate_snapshot

                nested = validate_snapshot(
                    nested_root,
                    "reconstruction" if payload.get("data_tier") == "cc0_reconstruction" else "release",
                )
            else:
                from scripts.data.document_contract import validate_document_snapshot

                nested = validate_document_snapshot(
                    nested_root,
                    "reconstruction" if payload.get("data_tier") == "cc0_reconstruction" else "release",
                )
    ...
    if not isinstance(readiness, dict) or set(readiness) != {"market", "documents"}:
        ...
        for name, item in readiness.items():
            if not isinstance(item, dict) or set(item) != {
                ...
                issues.append(ArtifactIssue("readiness_shape", name))
                continue
            candidate = _safe_child(bundle_root, item["path"], issues)
            if candidate is None:
                continue
            if not candidate.is_file():
                issues.append(ArtifactIssue("readiness_missing", name))
                continue
            if sha256_file(candidate) != item["sha256"]:
            ...
            if readiness_rows[name]:
                ...
                contracts_digest = hashlib.sha256(
                    json.dumps(
                        contract_bindings,
                        sort_keys=True,
                        separators=(",", ":"),
                ...
                if bank_digests != {item.get("bank_sha256")} or contracts_digest != item.get(
                    "contracts_sha256"
                ):
    ...
    if not isinstance(semantic, dict) or set(semantic) != {
        ...
        if (
            ...
            or type(semantic.get("eligible_document_count")) is not int
            or len(identifiers) != semantic.get("eligible_document_count")
        ...
        if semantic.get("status") == "ready":
            ...
            if (
                ...
                or not index_candidate.is_file()
                or sha256_file(index_candidate) != semantic.get("index_sha256")
            ):
    ...
    if not isinstance(artifacts, list):
        issues.append(ArtifactIssue("artifact_shape", "scenario-v2"))
        artifacts = []
    for item in artifacts:
        if not isinstance(item, dict) or set(item) != ARTIFACT_KEYS:
            issues.append(ArtifactIssue("artifact_shape", repr(item)))
            ...
            try:
                if candidate.suffix == ".jsonl":
                    observed_records = sum(
                        1 for line in candidate.read_text(encoding="utf-8").splitlines() if line.strip()
                    )
    ...
    if semantic.get("status") == "ready" and not semantic_artifacts_valid(
        artifacts,
        semantic.get("eligible_document_count"),
        root=bundle_root,
    ):
    ...
    if market_coverage != market.get("snapshot_id") or coverage.get("document_snapshot_id") != documents.get(
        "snapshot_id"
    ):
    ...
    expected_coverage = {
        ...
        "optional_gaps": [
            gap for gap in nested_market.get("quality", {}).get("gaps", []) if not gap.get("required")
        ],
    ...
    try:
        normalized_documents = [
            json.loads(line)
            for line in (bundle_root / "processed/documents.jsonl").read_text(encoding="utf-8").splitlines()
            if line.strip()
        ]
