...
def register_gateway(cli, env):
    ...
    if matches:
        if len(matches) != 1 or matches[0].get('endpoint') != 'http://127.0.0.1:17671' or matches[0].get('is_remote'):
            raise RuntimeError('Existing market-shock gateway registration does not match the approved local endpoint')
        return
...
def prepare_network():
    ...
    existing = run(['docker', 'network', 'ls', '--filter', f'name=^{name}$', '--format', '{{.Name}}']).decode().splitlines()
    if existing and existing != [name]:
        raise RuntimeError('OpenShell network identity is ambiguous')
    if not existing:
        run(['docker', 'network', 'create', '--driver', 'bridge', '--subnet', '172.22.0.0/16',
             '--gateway', '172.22.0.1', '--label', 'com.nvidia.market-shock.owner=openshell', name])
    ...
    if (network.get('Name') != name or network.get('Driver') != 'bridge'
            or network.get('Internal') or len(configs) != 1
            or configs[0].get('Subnet') != '172.22.0.0/16'
            or configs[0].get('Gateway') != '172.22.0.1'):
        raise RuntimeError('Existing OpenShell bridge does not match the pinned callback network; no network was replaced')
...
def main():
    ...
    env = os.environ.copy()
    for key, directory in [('XDG_CONFIG_HOME', 'config'), ('XDG_DATA_HOME', 'data'), ('XDG_STATE_HOME', 'state')]:
...
if __name__ == '__main__':
    try:
        ...
        raise SystemExit(str(error) if isinstance(error, RuntimeError) else 'OpenShell infrastructure preparation failed') from None
