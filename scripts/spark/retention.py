...
STATE_FILES = {'agent.key', 'switchyard-routing.jsonl', MARKER} | {
    name + suffix for name in ('investigations.sqlite3', 'checkpoints.sqlite3')
    for suffix in ('', '-wal', '-shm', '-journal')
}
GUIDANCE = ('Stop with ./demo stop; inventory: python3 scripts/spark/retention.py inventory '
...
def _runtime_directory_fd(root):
    ...
    try:
        ...
        if (not stat.S_ISDIR(observed.st_mode) or identity(observed) != identity(bound)
...
def _validate_runtime_lock_fd(fd, root):
    ...
    if (not stat.S_ISDIR(info.st_mode) or info.st_uid != os.geteuid()
            or stat.S_IMODE(info.st_mode) & 0o022 or identity(info) != identity(bound)):
...
def acquire_retention_lock(root=ROOT):
    ...
    try:
        lock_fd = os.open(
            LOCK, os.O_CREAT | os.O_RDWR | os.O_NOFOLLOW | os.O_CLOEXEC,
...
def agent_stopped():
    env = dict(os.environ)
    for name, suffix in (('XDG_CONFIG_HOME', 'config'), ('XDG_DATA_HOME', 'data'), ('XDG_STATE_HOME', 'state')):
        env[name] = str(ROOT / 'openshell' / suffix)
    try:
        result = subprocess.run(
            [str(ROOT / 'openshell/0.0.116/openshell'), '-g', 'market-shock',
             'sandbox', 'get', 'market-agent', '--output', 'json'],
            env=env, capture_output=True, timeout=30, check=True, text=True)
        if json.loads(result.stdout).get('phase') != 'Stopped':
            return False
        identifiers = subprocess.run(
            ['docker', 'ps', '--quiet', '--no-trunc'], capture_output=True,
            timeout=15, check=True, text=True,
        ).stdout.splitlines()
        if not identifiers:
            return True
        census = json.loads(subprocess.run(
        ...
        for container in census:
            ...
            for mount in mounts:
                ...
                if any(source_path == target or source_path in target.parents or target in source_path.parents
...
class Retention:
    def __init__(self, root=ROOT, *, stopped=agent_stopped, clock=time.time, test_root=False):
        self.root = Path(root)
        if not self.root.is_absolute() or self.root.resolve() != self.root or (not test_root and self.root != ROOT):
    ...
    def marker(self, base):
        ...
        if (path.stat().st_mode & 0o777 != 0o600 or path.stat().st_uid != os.geteuid()
                ...
                or value['classification'] != 'visitor' or str(uuid.UUID(value['id'])) != value['id']
    ...
    def inventory(self, *, hashes=True):
        ...
        if parent.exists() or parent.is_symlink():
            ...
            for path in sorted(parent.iterdir()):
                ...
                if (set(record) != {'id', 'created', 'classification', 'files'}
                        or record.get('id') != path.name or record.get('classification') not in ('historical', 'visitor')
                        or files != expected or set(p.name for p in path.iterdir()) != {'state', 'traces', 'manifest.json'}):
        ...
        return dict(schema_version=1, root=str(self.root), classification='visitor' if marker else 'historical',
                    active=rows, archives=archives)

    def expiry_targets(self, inventory):
        rows = sorted((a for a in inventory['archives'] if a['classification'] == 'visitor'),
                      key=lambda a: (a['created'], a['id']))
        targets = []
        while rows and (len(rows) > MAX_ARCHIVES
    ...
    def reset(self, manifest, *, apply=False):
        if manifest != self.inventory():
            raise RetentionError('prior manifest changed; inventory again')
        noop = manifest['classification'] == 'visitor' and all(r['path'] == f'state/{MARKER}' for r in manifest['active'])
        result = dict(apply=apply, noop=noop, classification=manifest['classification'], targets=['state', 'traces'])
        ...
        record = dict(id=archive_id, created=created, classification=manifest['classification'], files=self.files(archive))
        write_json(archive / 'manifest.json', record)
        for area in ('state', 'traces'):
            (self.root / area).mkdir(mode=0o700)
        write_json(self.root / 'state' / MARKER, dict(id=str(uuid.uuid4()), created=self.clock(), classification='visitor'))
        pending.unlink()
            ...
            for area in ('state', 'traces'):
                ...
                try:
                    for row in (r for r in record['files'] if r['path'].startswith(area + '/')):
                        ...
                        before = path.lstat()
                        if (read_file(path, digest=True) != row['sha256'] or before.st_size != row['size']
                                or stat.S_IMODE(before.st_mode) != row['mode'] or identity(before) != identity(path.lstat())):
...
def main():
    try:
        ...
        error = sys.exception()
        message = str(error) if isinstance(error, RetentionError) else 'retention inventory/operation failed closed'
