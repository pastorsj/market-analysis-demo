...
def environment(**updates):
    return {"REMOTE_ROUTING_ENABLED": "true", "NVIDIA_BASE_URL": "https://inference.example/v1",
            "NVIDIA_INFERENCE_API_KEY": "secret-inference", "NEMO_RELAY_LANGSMITH_ENABLED": "true",
            "LANGSMITH_ENDPOINT": "https://api.smith.langchain.com", "LANGSMITH_API_KEY": "secret-langsmith",
...
def test_endpoint_and_python_scoped_profiles():
    ...
    assert profile["endpoints"] == [{"host": "inference.example", "port": 443,
                                       "path": "/v1/**", "protocol": "rest", "access": "full",
                                       "enforcement": "enforce"}]
    ...
    granted_hosts = {
        endpoint["host"]
        for item in policy["network_policies"].values()
...
def test_disabled_remote_and_telemetry_have_no_network_grants():
    safe, policy, profiles = config.prepare_inputs(environment(REMOTE_ROUTING_ENABLED="false",
        NEMO_RELAY_LANGSMITH_ENABLED="false", LANGSMITH_PROJECT_URL=""), {"network_policies": {}})
    assert profiles == [] and policy["network_policies"] == {}
    assert not config.SECRET_KEYS.intersection(safe)


@pytest.mark.parametrize("updates", [
    {"NVIDIA_INFERENCE_API_KEY": ""}, {"NVIDIA_BASE_URL": ""},
    {"REMOTE_ROUTING_ENABLED": "maybe"}, {"UNREVIEWED_TOKEN": "secret"},
...
def test_credentials_only_in_child_environment(monkeypatch):
    ...
    config.openshell("provider", "create", "--credential", "LANGSMITH_API_KEY",
...
def test_command_failure_does_not_disclose_child_output(monkeypatch, capsys):
    monkeypatch.setattr(subprocess, "run", lambda *args, **kwargs:
                        subprocess.CompletedProcess(args, 1, "secret-stdout", "secret-stderr"))
