...
def _report(attempts: tuple[ModelAttempt, ...], final: ModelAttempt | None = None) -> FinalReport:
    selected = final or next(
        item for item in reversed(attempts) if item.state == "succeeded" and item.role == "agent_reasoning"
    )
    routing = legacy_routing_projection(selected, "switchyard_escalation").model_copy(
        update={"remote_attempted": True}
    )
    return FinalReport(
...
def test_report_accepts_one_adjacent_remote_transport_retry_per_request():
    judge = _success(
        request_id="request-judge-0001",
        call_id="call-judge-success",
        role="routing_judge",
        model=LUNA_MODEL,
        tier="judge",
    )
    ...
    report = _report(
        (
            _failed(judge, call_id="call-judge-failed"),
            judge,
            _failed(capable, call_id="call-capable-failed"),
            capable,
        )
    )
...
def test_report_rejects_every_other_duplicate_request_shape(case: str):
    ...
    if case == "wrong_failure":
        ...
        attempts = (
            failed.model_copy(update={"tokens": failed.tokens.model_copy(update={"prompt": 1, "total": 1})}),
            success,
        )
    elif case == "wrong_target":
        other = _success(
            request_id=success.application_request_id, call_id=success.application_call_id, model=SOL_MODEL
        )
        ...
        local = _success(
            request_id=success.application_request_id,
            call_id=success.application_call_id,
            model=LOCAL_MODEL,
            tier="efficient",
        )
        attempts, final = (_failed(local, call_id=failed.application_call_id), local), local
    elif case == "direct_retry":
        direct = success.model_copy(
            update={"role": "answer_synthesis", "algorithm": "direct_frontier", "selected_tier": None}
        )
