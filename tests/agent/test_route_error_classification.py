...
@pytest.mark.parametrize(
    ...
    [
        ...
        RuntimeError(
            "internal error: OpenAIInvalidRequestError: Error code: 400 - maximum context length is 32768 tokens"
        ),
    ],
)
...
async def test_observer_records_only_safe_classification_for_context_error():
    ...
    serialized = json.dumps(
        {"events": captured, "attempt": observer.attempts[0].model_dump(mode="json")}, default=str
    )
...
async def test_remote_connection_failure_retries_once_with_distinct_audit_rows():
    ...
    async def invoke():
        ...
        if calls == 1:
            ...
            error = APIConnectionError(request=httpx.Request("POST", "https://inference.example.test/v1"))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        RuntimeError("Unable to complete request: max_output_tokens; received model group=capable"),
    ],
)
