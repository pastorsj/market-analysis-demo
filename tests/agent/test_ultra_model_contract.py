...
def test_ultra_default_and_no_sol_runtime_fallback():
    ...
    for unsupported in (SOL_MODEL, "nvidia/nvidia/llama-3.1-nemotron-ultra-253b-v1", "nvidia/nvidia/nemotron-3-ultra-evals"):
...
def test_current_and_historical_capable_receipts_remain_readable(model):
    receipt = dict(role="agent_reasoning", algorithm="switchyard_escalation",
                   destination_class="internal_inference", configured_model=model,
                   model_assertion=model, identity_evidence="direct_provider_verified",
                   state="succeeded", application_call_id="call-ultra-test",
                   application_request_id="request-ultra-test", latency_ms=1,
                   validation_status="valid", selected_tier="capable")
